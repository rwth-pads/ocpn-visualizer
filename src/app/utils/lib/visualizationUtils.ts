...
export async function visualizeOCPN(layout: OCPNLayout, config: OCPNConfig, svgRef: SVGSVGElement | null) {
    ...
    for (const vertexId in layout.vertices) {
        ...
        if (vertex.type === OCPNLayout.PLACE_TYPE) {
            ...
            // Just for debugging the dummy vertices are not visible in the final implementation.
            g.append('circle')
                .attr('cx', vertex.x)
                .attr('cy', vertex.y)
                .attr('r', 3) // TODO: user defined radius
                .attr('arcId', vertex.belongsTo)
                .attr('dummyId', vertexId)
                .attr('upper', vertex.upper)
                .attr('lower', vertex.lower)
                .attr('fill', 'green')
                .attr('class', 'ocpndummy')
                .attr('id', vertexId);

            // On hover, show a tooltip with the arcId.
            ...
            // On hover, show a tooltip with the arcId.
            g.selectAll('.ocpndummy')
                .on('mouseover', function (event, d) {
                    const arcId = d3.select(this).attr('arcId');
                    const dummyId = d3.select(this).attr('dummyId');
                    const upper = d3.select(this).attr('upper');
                    const lower = d3.select(this).attr('lower');
                    tooltip.text(`ArcId: ${arcId}, DummyId: ${dummyId}, Upper: ${upper}, Lower: ${lower}`)
                        .style('visibility', 'visible');

                    // Highlight the corresponding arc red.
                    d3.select(`#${arcId}`)
                        .attr('stroke', 'red')
                        .attr('stroke-width', 2);
                    // Hihglight the upper and lower red.
                    d3.select(`#${upper}`)
                        .attr('fill', 'red')
                    d3.select(`#${lower}`)
                        .attr('fill', 'red');
                ...
                .on('mouseout', function () {
                    ...
                    d3.select(`#${arcId}`)
                        .attr('stroke', 'black')
                        .attr('stroke-width', 1);
                    // Reset the upper and lower color.
                    const upper = d3.select(this).attr('upper');
                    d3.select(`#${upper}`)
                        .attr('fill', 'black');
                    const lower = d3.select(this).attr('lower');
